...
Про учёт времени: заблокированный период считается активным (см.
`events_monitor.set_input_lock_active`) — блокировка означает «я на месте,
просто ввод сейчас не нужен», а не «я ушёл». Сами нажатия при этом
проглатываются фильтром и на счётчик активности не влияют.
